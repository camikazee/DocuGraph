...
- [x] **Backups** — documented backup/restore in `DEPLOY.md §7` (Mongo logical dump + workspace/media volume tar + the secrets you must keep to decrypt it), plus runnable `scripts/backup.sh` (read-only, timestamped) and `scripts/restore.sh` (destructive, confirmation-gated). Covers cadence, retention/offsite, managed-Mongo, and a "test your restore" reminder.
- [x] **Secrets** — production secret management without a plaintext `.env`: the backend supports the `<NAME>_FILE` convention (Docker/Swarm/K8s secrets mounted as files → loaded into env at startup, explicit env wins, trailing newline stripped), covered by a unit test. `DEPLOY.md §1` documents it with Compose/Swarm/Portainer/K8s examples and the "`MEDIA_SECRET` must stay stable" caveat.
...
- [x] **Fix e2e DB isolation** — e2e run on an in-memory Mongo per test file (`mongodb-memory-server`); no external DB, parallel-safe, and `MONGO_URI_TEST` still overrides to a real Mongo
...
- [x] **Atomic content persistence diagnostics** — Markdown writes use fsync + atomic rename; Owner-only consistency diagnostics report files missing on disk, rows missing from the index, and content mismatches before an explicit reindex
- [x] **Durable side effects without extra infrastructure** — watcher e-mails and automatic Git publication use Mongo-backed jobs with idempotency keys, leases, bounded retries, and sanitized errors; Redis/a queue broker remains optional
- [x] **Post-deploy verification** — Jenkins can run a retrying, read-only smoke for frontend, liveness, readiness, and optional authenticated docs health after a Portainer webhook
...
- [x] **Branded export** — exports are titled with the workspace name (single-file + ZIP), and the reader has a **Save as PDF** button: a print stylesheet strips the app chrome and prints a clean, branded (workspace + path) document via the browser's own PDF engine — no headless-Chromium dependency. *(A server-side bulk/branded PDF of the whole site via Chromium remains an optional upgrade if ever needed.)*
...
- [x] **Frontend test coverage** (components + key flows) — Jest/RTL covers validators, notification/access helpers, invite acceptance, ShareDialog, auth/dashboard/account flows, the API transport boundary, stale-request cancellation, and dialog focus containment. Runs in CI. *(Deeper flow coverage can keep growing.)*
- [x] **Accessibility audit** (keyboard, focus, contrast, reduced motion) — a global `:focus-visible` ring (placed to win over `outline-none`) gives every interactive element a visible keyboard focus indicator; an app-wide `prefers-reduced-motion` block neutralizes transitions/animations; and the two custom `role="button"` controls (tag chip, volume reconnect) got Enter/Space handlers plus `aria-label`s. Contrast rides on the existing themed tokens (light/grey/violet). *(A formal WCAG contrast pass can still follow.)*
- [x] **Mobile/responsive polish** — the app shell (used by every main page) now collapses to an off-canvas drawer behind a hamburger top bar below `lg`, with a backdrop and auto-close on navigation; the graph's three-column layout stacks; wide tables (documents, team) scroll horizontally inside their own container instead of breaking the page; and the documents header actions wrap. *(The full-screen reader/editor 3-column layouts still deserve a dedicated mobile pass.)*
